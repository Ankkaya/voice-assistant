...
本项目为 6～9 岁儿童提供一个模拟打电话的语音对话 App。孩子在角色选择页选择卡通人物，进入通话页后无需按键说话；Flutter 在本地检测开始说话和连续静音，通过 WebSocket 将 PCM 音频流发送到后端。后端使用 MiMo ASR、LangChain Agent 和 MiMo TTS 生成回答，并把语音分片返回 App 播放。
...
回答要求：面向 6～9 岁、简单中文、最多三句话、最多 80 个汉字、一次最多一个问题、不输出 Markdown、网址或复杂符号。
